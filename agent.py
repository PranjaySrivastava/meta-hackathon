...
class TextAgent:
    ...
    State schema:
        ...
        intent      : str   – pre-detected intent label (may be "")
        sentiment   : str   – "positive" | "negative" | "neutral" | ""
        confidence  : float – classifier confidence 0.0–1.0
        context     : list  – prior [state, action] pairs (may be [])
    """
    ...
    def _rule_based(self, state: dict) -> str:
        ...
        # 2. Very low confidence + vague text → clarify
        if confidence < 0.3:
            return "clarify"
        ...
        # 4. Sarcasm: positive words + failure context → apologize_and_fix
        ...
        # 7. Low-medium confidence + ambiguous → clarify
